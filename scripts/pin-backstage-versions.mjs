...
const backstagePackageJson = await fetch(backstagePackageJsonUrl).then(res =>
  res.json(),
);
...
for (const pkg of versionManifestJson.packages) {
  if (resolutions[pkg.name] === pkg.version) {
    console.log(
      `Package ${pkg.name} already has the correct resolution (${pkg.version}), skipping`,
    );
    continue;
  } else if (resolutions[pkg.name]) {
    console.log(
      `Package ${pkg.name} has a different resolution (${
        resolutions[pkg.name]
      }) than the version manifest (${pkg.version}), updating to ${
        pkg.version
      }`,
    );
  } else {
    console.log(
      `Package ${pkg.name} has no resolution, pinning to ${pkg.version}`,
    );
  }
...
if (backstagePackageJson.resolutions['GendocuPublicApis']) {
  if (
    resolutions['GendocuPublicApis'] ===
    backstagePackageJson.resolutions['GendocuPublicApis']
  ) {
    console.log(
      `GendocuPublicApis resolution is already correct (${resolutions['GendocuPublicApis']}), skipping`,
    );
  } else {
    console.log(
      `GendocuPublicApis resolution is ${resolutions['GendocuPublicApis']}, but should be ${backstagePackageJson.resolutions['GendocuPublicApis']}, updating`,
    );
    resolutions['GendocuPublicApis'] =
      backstagePackageJson.resolutions['GendocuPublicApis'];
  }
